...
def plot_sft_curves(
    ...
    if thinking_rate:
        ...
        if len(thinking_rate) > 0:
            ax3.annotate(f'{thinking_rate[-1]:.1%}',
                        xy=(eval_steps[-1], thinking_rate[-1]),
                        xytext=(5, 5), textcoords='offset points',
                        fontsize=10, color='green')
    else:
        ax3.text(0.5, 0.5, 'No eval data available',
    ...
    if avg_response_length:
        ax5.plot(eval_steps, avg_response_length, 'tab:cyan', linewidth=2, marker='o', markersize=6, label='Avg Response Length')
        ...
        if len(avg_response_length) > 0:
            ax5.annotate(f'{avg_response_length[-1]:.0f}',
                        xy=(eval_steps[-1], avg_response_length[-1]),
                        xytext=(5, 5), textcoords='offset points',
                        fontsize=10, color='tab:cyan')
    else:
        ax5.text(0.5, 0.5, 'No eval data available',
    ...
    if avg_thinking_length:
        ax6.plot(eval_steps, avg_thinking_length, 'tab:brown', linewidth=2, marker='s', markersize=6, label='Avg Thinking Length')
        ...
        if len(avg_thinking_length) > 0:
            ax6.annotate(f'{avg_thinking_length[-1]:.0f}',
                        ...
                        xytext=(5, 5), textcoords='offset points',
                        fontsize=10, color='tab:brown')
    else:
        ax6.text(0.5, 0.5, 'No eval data available',
                ha='center', va='center', transform=ax6.transAxes, fontsize=12, color='gray')
        ax6.set_title('Average Thinking Content Length (Eval)')
...
def plot_simple(
    ...
    if thinking_rate:
        ...
        line2 = ax2.plot(eval_steps, thinking_rate, color=color2, linewidth=2,
                        marker='o', markersize=6, label='Thinking Rate')
    ...
    # 标注起始和结束值
    ax1.annotate(f'Start: {loss[0]:.4f}',
                xy=(steps[0], loss[0]),
...
def main():
    ...
    parser.add_argument("history_file", nargs="?", default=None,
                        help="history.json 文件路径（默认使用最新的）")
    parser.add_argument("--output-dir", type=str, default=None,
                        help="输出目录（默认与 history.json 同目录）")
    parser.add_argument("--simple", action="store_true",
                        help="只生成简洁的 Loss + Thinking Rate 曲线")
